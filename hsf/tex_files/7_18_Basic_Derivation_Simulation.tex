...
\begin{align*}
    ...
    \\
    \implies
    S^{-1}_j \mathcal{U}_j^T e &> \frac{S^{-2}_j}{2}\\
    \implies
     S_j \mathcal{U}_j^T e &> \frac{1}{2}\\
    \implies
    v_j &> \frac{1}{2},{
	\frac
	{
...
\label{eq:voltage_threshold}
    ...
    &\text{if  } v_j > v^{th}_j,\notag \\
    \\
    &\text{then  } v_j^{'} = v_j - \int \delta(\tau)  d\tau ,\notag \\
    \notag \\ 
    &\text{and  } \rho_j^{'} = \rho_j + \int \delta(\tau)  d\tau \notag
\end{align}
where  
\begin{align*}
v_j^{'} &= lim_{\xi \to \xi_{spike}^ {+}} v_j(\xi),\\
\\
\rho_j^{'} &= lim_{\xi \to \xi_{spike}^ {+}} \rho_j(\xi),\\
\\
\end{align*}
and  $1_N$ is the N-vector with entries 1. 
